...
GUI::GUI() :
		depthImg(0), depthImgBuffer(0) {
	pangolin::CreateGlutWindowAndBind("Main", 1280 + 180, 960,
	...
	pangolin::Display("cam").SetBounds(0, 1.0f, 0, 1.0f, -640 / 480.0).SetHandler(
			new pangolin::Handler3D(s_cam));

	pangolin::Display("Image").SetAspect(640.0f / 480.0f);

	pangolin::Display("multi").SetBounds(pangolin::Attach::Pix(0), 1 / 4.0f,
			pangolin::Attach::Pix(180), 1.0).SetLayout(
			pangolin::LayoutEqualHorizontal).AddDisplay(
...
void GUI::updateKeyframePoses(GraphFramePose* framePoseData, int num) {
	...
	for (int i = 0; i < num; i++) {
		if (keyframes.getReference().find(framePoseData[i].id)
				!= keyframes.getReference().end()) {
			memcpy(
					keyframes.getReference()[framePoseData[i].id]->camToWorld.data(),
...
void GUI::drawFrustum() {
	...
	glVertex3f(0, 0, 0);
	glVertex3f(
			0.05 * (0 - Intrinsics::getInstance().cx())
					/ Intrinsics::getInstance().fx(),
			0.05 * (0 - Intrinsics::getInstance().cy())
					/ Intrinsics::getInstance().fy(), 0.05);
	glVertex3f(0, 0, 0);
	glVertex3f(
			0.05 * (0 - Intrinsics::getInstance().cx())
					/ Intrinsics::getInstance().fx(),
			0.05
					* (Resolution::getInstance().height() - 1
							- Intrinsics::getInstance().cy())
					/ Intrinsics::getInstance().fy(), 0.05);
	glVertex3f(0, 0, 0);
	glVertex3f(
			0.05
					* (Resolution::getInstance().width() - 1
