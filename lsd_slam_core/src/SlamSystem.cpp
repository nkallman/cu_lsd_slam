...
SlamSystem::SlamSystem(int w, int h, Eigen::Matrix3f K, bool enableSLAM) :
	...
	if (SLAMEnabled) {
		trackableKeyFrameSearch = new TrackableKeyFrameSearch(keyFrameGraph, w,
				h, K);
	...
	if (SLAMEnabled) {
		...
		thread_optimization = boost::thread(&SlamSystem::optimizationThreadLoop,
				this);
	}
	...
	while (keepRunning) {
		if (newKeyFrames.size() == 0) {
			...
			if (keyFrameGraph->keyframesForRetrack.size() > 10) {
				std::deque<Frame*>::iterator toReTrack =
						keyFrameGraph->keyframesForRetrack.begin()
								+ (rand()
										% (keyFrameGraph->keyframesForRetrack.size()
												/ 3));
				...
				int found = findConstraintsForNewKeyFrames(toReTrackFrame,
						false, false, 2.0);
				if (found == 0)
					failedToRetrack++;
				else
			...
			if (!doneSomething) {
				...
				newKeyFrameCreatedSignal.timed_wait(lock,
		...
		if (doFullReConstraintTrack) {
			...
			int added = 0;
			for (unsigned int i = 0; i < keyFrameGraph->keyframesAll.size();
					i++) {
...
void SlamSystem::finishCurrentKeyframe() {
	...
	if (SLAMEnabled) {
		...
		if (currentKeyFrame->idxInKeyframes < 0) {
			...
			currentKeyFrame->idxInKeyframes =
...
void SlamSystem::discardCurrentKeyframe() {
	...
	if (currentKeyFrame->idxInKeyframes >= 0) {
		printf(
				"WARNING: trying to discard a KF that has already been added to the graph... finalizing instead.\n");
...
void SlamSystem::createNewCurrentKeyframe(
	...
	if (SLAMEnabled) {
		...
		keyFrameGraph->idToKeyFrame.insert(
				std::make_pair(newKeyframeCandidate->id(),
	...
	if (newReferenceKF != 0)
		...
		if (force) {
			if (noCreate) {
				...
				nextRelocIdx = -1;
				printf(
						"mapping is disabled & moved outside of known map. Starting Relocalizer!\n");
			} else
...
void SlamSystem::addTimingSamples() {
	...
	if (sPassed > 1.0f) {
		...
		if (trackableKeyFrameSearch != 0) {
			trackableKeyFrameSearch->nAvgTrackPermaRef = 0.8
		...
		if (enablePrintDebugInfo && printOverallTiming) {
			printf(
					"MapIt: %3.1fms (%.1fHz); Track: %3.1fms (%.1fHz); Create: %3.1fms (%.1fHz); FindRef: %3.1fms (%.1fHz); PermaTrk: %3.1fms (%.1fHz); Opt: %3.1fms (%.1fHz); FindConst: %3.1fms (%.1fHz);\n",
					map->msUpdate, map->nAvgUpdate, msTrackFrame,
					nAvgTrackFrame, map->msCreate + map->msFinalize,
					map->nAvgCreate, msFindReferences, nAvgFindReferences,
					trackableKeyFrameSearch != 0 ?
							trackableKeyFrameSearch->msTrackPermaRef : 0,
					trackableKeyFrameSearch != 0 ?
							trackableKeyFrameSearch->nAvgTrackPermaRef : 0,
...
void SlamSystem::randomInit(uchar* image, double timeStamp, int id) {
	...
	if (!doMapping)
		printf(
				"WARNING: mapping is disabled, but we just initialized... THIS WILL NOT WORK! Set doMapping to true.\n");
...
void SlamSystem::trackFrame(uchar* image, unsigned int frameID,
	...
	if (manualTrackingLossIndicated || tracker->diverged
		...
		printf(
				"TRACKING LOST for frame %d (%1.2f%% good Points, which is %1.2f%% of available points, %s)!\n",
	...
	if (unmappedTrackedFrames.size() < 50
			|| (unmappedTrackedFrames.size() < 100
					&& trackingNewFrame->getTrackingParent()->numMappedOnThisTotal
							< 10))
...
float SlamSystem::tryTrackSim3(TrackingReference* A, TrackingReference* B,
		int lvlStart, int lvlEnd, bool useSSE, Sim3 &AtoB, Sim3 &BtoA,
		KFConstraintStruct* e1, KFConstraintStruct* e2) {
	BtoA = constraintTracker->trackFrameSim3(A, B->keyframe, BtoA, lvlStart,
			lvlEnd);
	...
	if (constraintTracker->diverged
			|| BtoA.scale() > 1 / Sophus::SophusConstants < sophusType
					> ::epsilon()
	...
	AtoB = constraintTracker->trackFrameSim3(B, A->keyframe, AtoB, lvlStart,
			lvlEnd);
	...
	if (constraintTracker->diverged
			|| AtoB.scale() > 1 / Sophus::SophusConstants < sophusType
					> ::epsilon()
	...
	Matrix7x7 diffHesse =
			(AtoBInfo.inverse()
					+ datimesb_db * BtoAInfo.inverse() * datimesb_db.transpose()).inverse();
...
void SlamSystem::testConstraint(Frame* candidate, KFConstraintStruct* &e1_out,
	...
	Sim3 FtoC = candidateToFrame_initialEstimate.inverse(), CtoF =
	...
	if (enablePrintDebugInfo && printConstraintSearchInfo)
		printf("ADDED %d -> %d: errs (%.1f / %.1f / %.1f).",
				newKFTrackingReference->frameID,
				candidateTrackingReference->frameID, sqrtf(err_level3),
				sqrtf(err_level2), sqrtf(err_level1));
...
int SlamSystem::findConstraintsForNewKeyFrames(Frame* newKeyFrame,
	...
	if (!forceParent
			&& (newKeyFrame->lastConstraintTrackedCamToWorld
					* newKeyFrame->getScaledCamToWorld().inverse()).log().norm()
					< 0.01)
		return 0;
	...
	std::vector<KFConstraintStruct*,
			Eigen::aligned_allocator<KFConstraintStruct*> > constraints;
	Frame* fabMapResult = 0;
	std::unordered_set<Frame*, std::hash<Frame*>, std::equal_to<Frame*>,
			Eigen::aligned_allocator<Frame*> > candidates =
	...
	std::map<Frame*, Sim3, std::less<Frame*>,
			Eigen::aligned_allocator<std::pair<Frame*, Sim3> > > candidateToFrame_initialEstimateMap;
	...
	for (Frame* candidate : candidates) {
		...
		SE3 c2f = constraintSE3Tracker->trackFrameOnPermaref(candidate,
				newKeyFrame, c2f_init);
		...
		SE3 f2c = constraintSE3Tracker->trackFrameOnPermaref(newKeyFrame,
				candidate, f2c_init);
		if (!constraintSE3Tracker->trackingWasGood) {
			closeFailed++;
			continue;
	...
	if (enablePrintDebugInfo && printConstraintSearchInfo)
		printf(
				...
				closeInconsistent, (int) farCandidates.size(), farAll,
				farFailed, farInconsistent,
	...
	if (parent != 0 && forceParent) {
		...
		if (e1 != 0) {
			...
			constraints.back()->information *= (1e9
					/ (downweightFac * downweightFac));
...
Sophus::Sim3f SlamSystem::getCurrentPoseEstimateScale() {
	...
	keyFrameGraph->allFramePosesMutex.lock_shared();
	if (keyFrameGraph->allFramePoses.size() > 0)
		camToWorld = keyFrameGraph->allFramePoses.back()->getCamToWorld().cast<
				float>();
	keyFrameGraph->allFramePosesMutex.unlock_shared();
	return camToWorld;
}
